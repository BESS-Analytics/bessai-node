...
export class AgentResource {
  ...
  async create(params: AgentCreateParams): Promise<AgentResponse> {
    return this.client.post<AgentResponse>("/v1/agents", {
  ...
  async list(skip = 0, limit = 50): Promise<PaginatedResponse<AgentResponse>> {
    return this.client.get<PaginatedResponse<AgentResponse>>("/v1/agents", {
      params: { skip, limit },
    });
  }
  ...
  async update(agentId: string, params: AgentUpdateParams): Promise<AgentResponse> {
    return this.client.patch<AgentResponse>(`/v1/agents/${agentId}`, {
  ...
  async publish(agentId: string, version?: number): Promise<AgentResponse> {
    ...
    return this.client.post<AgentResponse>(`/v1/agents/${agentId}/publish`, {
      params,
    });
  }
  ...
  async linkWorkflow(
    ...
    return this.client.post(`/v1/agents/${agentId}/workflows/${workflowId}`, {
      json: options
        ? {
  ...
  async unlinkWorkflow(
    ...
    return this.client.delete(`/v1/agents/${agentId}/workflows/${workflowId}`);
  }

  /** List workflows linked to an agent. */
  async listWorkflows(agentId: string): Promise<Record<string, unknown>[]> {
    return this.client.get<Record<string, unknown>[]>(`/v1/agents/${agentId}/workflows`);
  }
}
